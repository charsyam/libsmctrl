...
//    (SM ID range check is skipped on Blackwell+ where SM-to-TPC mapping
//     is non-contiguous; only the SM count is verified)
int test_constrained_size_and_location(enum partitioning_type part_type) {
  ...
  for (int enabled_tpc = 0; enabled_tpc < num_tpcs && enabled_tpc < 128; enabled_tpc += 32) {
    ...
    // On Blackwell+ (cap_major >= 10), SM-to-TPC mapping is non-contiguous
    // (e.g., TPC 1 may map to SM 8,9 instead of SM 2,3), so we only check
    // the SM count above and skip the SM ID range verification.
    if (cap_major < 10) {
